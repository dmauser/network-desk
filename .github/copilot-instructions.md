...
- **Validation-first MCP policy (centralized).** The extension treats the Microsoft Learn MCP server as the primary source of truth for **Azure** facts. The directive lives in `registry.mjs` as `MCP_VALIDATION_DIRECTIVE` (+ `MCP_FALLBACK_BANNER` and the compact `MCP_VALIDATION_NOTE`) and is surfaced on every `cn_role`/`cn_skill` load (via `withPolicy()`), in `buildOrchestrator`, in the presence note, and in the prompt hook whenever a specialist trigger matches — and is mirrored into the generated plugin (coordinator agent + each specialist `SKILL.md`) by `build-plugin.mjs`. Azure facts must be Learn-validated ("Learn wins"; cite URLs) or fall back to a ⚠️ unverified banner with `copilot mcp add --transport http microsoft-learn https://learn.microsoft.com/api/mcp`. Learn is Azure-only — AWS/GCP/firewall facts keep the vendor-doc guardrail. Network Desk never edits MCP config (per-user CLI setting).
