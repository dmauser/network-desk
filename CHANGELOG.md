...
- **`vnet-architect` Excalidraw skill rewritten around crisp, technical-architecture rules.**
  The skill at `extensions/network-desk/specialists/vnet-architect/skills/excalidraw-diagram/SKILL.md`
  (and its generated mirror under `plugins/network-desk/skills/network-desk-vnet/reference/excalidraw-diagram/`)
  no longer teaches the sketchy / whiteboard defaults. New mandatory rules apply to every
  emitted element:
  - **Style:** `roughness: 0` (never 1 or 2), `strokeStyle: "solid"`, `strokeWidth: 1.5`,
    `roundness: null` for architecture rectangles, `fontFamily: 2` (Helvetica — never Virgil),
    `lineHeight: 1.15`, `fillStyle: "solid"` (no hachure / cross-hatch).
  - **Palette:** fixed 6-role palette (Azure primary `#0b3d6b`/`#eaf2fb`, Azure accent
    `#0078d4`/`#e6f0fb`, Gateway/success `#1f6b34`/`#ebf5ee`, Warning/NSG `#b08400`/`#fff5cc`,
    Neutral container `#495057`/`#f3f5f7`, body text `#1e1e1e`/`#5b5b5b`, canvas `#ffffff`).
  - **Layout:** 10px-grid snap, named tinted zones with 14pt titles, left-to-right flow,
    60px gap between zones, 30px between siblings, font hierarchy 24/14/13–14/11pt.
  - **Arrows:** `startBinding: null` and `endBinding: null` for all multi-segment connectors
    (bindings auto-reroute and produce wrong sweeps); 4-point elbow `points` as relative
    offsets; arrow labels rendered as separate 11pt `#5b5b5b` text elements.
  - **Full per-element schema is mandatory** (`seed`, `versionNonce`, `originalText`,
    `boundElements: []`, `updated: 1`, etc.) — minimal elements render blank.
  - **Output contract:** `.excalidraw` JSON, UTF-8, 2-space indent, top-level
    `appState.gridSize: null`, and the build must end with a `JSON.parse` validation step
    reporting element + arrow counts.
  - A fully-compliant minimal hub-spoke template ships in the skill as the starting point.
- **`registry.mjs`** Excalidraw skill description updated to advertise the crisp/technical
  posture so the router and capabilities tool surface the new behavior.

### Breaking

- Excalidraw diagrams generated by `vnet_skill_excalidraw_diagram` now look fundamentally
  different from prior versions (crisp/Helvetica/solid, not sketchy/Virgil/hachure).
  Existing diagrams on disk are not retroactively updated; only future generations follow
  the new rules.
